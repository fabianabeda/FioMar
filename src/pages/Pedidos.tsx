// pages/Pedidos.tsx (versão atualizada)
...
import {
    ...
    DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu"; // Importe o DropdownMenu
import { ArrowLeft, Plus, Search, Edit, Trash2, MoreVertical } from "lucide-react"; // Importe MoreVertical
import { toast } from "sonner";
import { format } from "date-fns";
import { ptBR } from "date-fns/locale";

// ... (interface Pedido e statusConfig continuam os mesmos) ...
interface Pedido {
    id: string;
    cliente_id: string;
    clientes: {
...
const statusConfig: Record<string, { label: string; color: string }> = {
    ...
    aguardando_pagamento: { label: "Aguardando Pagamento", color: "bg-orange-500" },
...
export default function Pedidos() {
    ...
    const filterPedidos = () => {
        ...
        if (searchTerm) {
            filtered = filtered.filter((p) => p.clientes.nome_completo.toLowerCase().includes(searchTerm.toLowerCase()) || p.produto.toLowerCase().includes(searchTerm.toLowerCase()));
        }
        if (statusFilter !== "all") {
    ...
    const handleStatusChange = async (pedidoId: string, newStatus: string) => {
        try {
            ...
            loadPedidos(); // Recarrega os pedidos para mostrar a mudança
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-background via-muted to-background">
            {/* ... (Header e Filtros continuam os mesmos) ... */}
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <Badge className={`${statusConfig[pedido.status]?.color || 'bg-gray-400'} text-white`}>
                                                {statusConfig[pedido.status]?.label || pedido.status}
                                            </Badge>
                                        </div>
                                        <div className="space-y-1 text-sm text-muted-foreground">
                                            <p><span className="font-medium">Produto:</span> {pedido.produto} - {pedido.modelo_cima} / {pedido.modelo_baixo}</p>
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                ...
                                                {Object.keys(statusConfig).map((statusKey) => (
                                                    <DropdownMenuItem
                                                        key={statusKey}
                                                        disabled={pedido.status === statusKey}
                                                        onClick={() => handleStatusChange(pedido.id, statusKey)}
                                                    >
                                                        {statusConfig[statusKey].label}
                                                    </DropdownMenuItem>
                                                ))}
                                                <DropdownMenuSeparator />
                                                <DropdownMenuItem className="text-red-600" onClick={() => handleDelete(pedido.id)}>
